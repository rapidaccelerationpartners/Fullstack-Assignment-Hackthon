    ...
    if (up) {
      await Promise.all(
        theRoot.trainsOnTheRoot.map(async (value) => {
          ...
          const arrivalTime =
            train.startTime + startStation.distance / train.speed;
          const departureTime =
            train.startTime + endStation.distance / train.speed;
      ...
      await Promise.all(
        theRoot.trainsOnTheRoot.map(async (value) => {
          const train = await Train.findOne({ _id: value });
          const arrivalTime =
            train.returnTime + (maxDis - startStation.distance / train.speed);
          const departureTime =
            train.returnTime + (maxDis - endStation.distance / train.speed);
...
exports.trainDetails = async (req, res) => {
  try {
    ...
    console.log(start,end,trainId)

    const startStation = await Station.findOne({ stationName: start });
    const endStation = await Station.findOne({ stationName: end });
    const theTrain = await Train.findOne({ _id: trainId });

    console.log(startStation,endStation,theTrain)
    ...
    if (up) {
      inBetweenStation.map((value) => {
        const arrivalTime =
          theTrain.startTime + value.distance / theTrain.speed;
      ...
      inBetweenStation.map((value) => {
        const arrivalTime =
          theTrain.returnTime + (maxDis - value.distance) / theTrain.speed;
